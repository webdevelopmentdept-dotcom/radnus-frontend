  ...
  return (
    <EmployeeLayout>
      <style>{`
        ...
        .sa-tab-btn { flex: 1 1 0; min-width: 0; padding: 8px 6px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; transition: all 0.2s; }
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 12 }}>
                      <div>
                        ...
                        <select value={logForm.kpi_item_id}
                          ...
                          {form.items.map(item => (
                            <option key={item.kpi_item_id} value={item.kpi_item_id}>
                              {item.kpi_name}{item.is_admission_kpi ? " 🎓" : ""} (target: {item.target} {item.unit})
                            </option>
                          ))}
                        </select>
                      </div>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, gap: 8 }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                                ...
                                {item.is_admission_kpi && <span className="adm-kpi-badge">🎓</span>}
                              </div>
                            </div>
                ...
                <div style={{ minWidth: 0 }}>
                    ...
                    return (
                      <div key={idx} style={{ background: "#fff", borderRadius: 12, marginBottom: 12, border: `1px solid ${item.is_admission_kpi ? "#bae6fd" : item.self_value !== "" ? "#e5e7eb" : "#fde68a"}`, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                        <div className="kpi-card-header" onClick={() => !isHRReviewed && setExpandedItem(isOpen ? null : idx)}
                          style={{ padding: "14px", cursor: isHRReviewed ? "default" : "pointer", display: "flex", justifyContent: "space-between", alignItems: "flex-start", background: isOpen ? "#f8fafc" : item.is_admission_kpi ? "#f0f9ff" : "#fff" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 10, flex: 1, minWidth: 0 }}>
                            <div style={{ width: 34, height: 34, borderRadius: 8, background: item.self_value !== "" ? "#f0fdf4" : "#fffbeb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0, marginTop: 2 }}>
                              {item.self_value !== "" ? "✅" : "⏳"}
                            </div>
                            <div style={{ minWidth: 0, flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 2 }}>
                                <p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: "#1f2937" }}>{item.kpi_name}</p>
                                {item.is_admission_kpi && <span className="adm-kpi-badge">🎓 Admission</span>}
                              </div>
                  ...
                  {isHRReviewed ? (
                    ...
                    <>
                      ...
                      {!isFormValid() && <p style={{ textAlign: "center", fontSize: 12, color: "#f59e0b", marginTop: 8 }}>⚠️ Please fill in all KPI values before submitting</p>}
                    </>
                  )}
                </div>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                              ...
                              {item.is_admission_kpi && <span style={{ fontSize: 9, background: "#f0f9ff", color: "#0369a1", padding: "1px 5px", borderRadius: 99, border: "1px solid #bae6fd", fontWeight: 700, flexShrink: 0 }}>🎓</span>}
                            </div>
                          </div>
