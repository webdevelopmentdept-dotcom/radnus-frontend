...
export default function MyClubs() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      // Fetch employee info for EmployeeLayout
      ...
      const results  = await Promise.all(
        clubKeys.map(key => Promise.all([
          axios.get(`${API_BASE}/api/clubs/${key}/my-membership?employee_id=${employeeId}`).catch(() => ({ data: { data: null } })),
          axios.get(`${API_BASE}/api/clubs/${key}/events`).catch(() => ({ data: { data: [] } })),
          axios.get(`${API_BASE}/api/clubs/${key}/points/my?employee_id=${employeeId}`).catch(() => ({ data: { data: [] } })),
        ]))
      );
  ...
  const handleJoin = async (clubKey) => {
    ...
    try {
      await axios.post(`${API_BASE}/api/clubs/${clubKey}/members`, { employee_id: employeeId });
  ...
  const allUpcoming      = Object.entries(allEvents)
    .flatMap(([key, evs]) => evs.filter(e => e.status === "upcoming" || e.status === "ongoing").map(e => ({ ...e, clubKey: key })))
  ...
  return (
    <EmployeeLayout employee={employee}>
      <div style={{ fontFamily: "'Segoe UI',sans-serif", minHeight: "100vh", background: "#f3f4f6" }}>
        ...
        {/* Toast */}
        {toast && (
          <div style={{ position: "fixed", top: 18, right: 16, zIndex: 9999, background: toast.type === "error" ? "#ef4444" : "#10b981", color: "#fff", padding: "11px 16px", borderRadius: 9, fontWeight: 600, fontSize: 13, boxShadow: "0 8px 24px rgba(0,0,0,.15)", display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <div key={cfg.key} className="club-card" style={{ background: "#fff", borderRadius: 14, border: `1.5px solid ${status === "approved" ? cfg.color + "44" : "#e5e7eb"}`, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,.06)" }}>
                      ...
                      <div style={{ padding: "18px" }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          ...
                          {!status && (
                            <button onClick={() => handleJoin(cfg.key)} disabled={joiningClub === cfg.key} style={{ flex: 1, padding: "8px 12px", background: cfg.color, border: "none", borderRadius: 8, color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, opacity: joiningClub === cfg.key ? .6 : 1 }}>
                ...
                <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: "18px 20px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {allUpcoming.slice(0, 8).map(ev => {
                      ...
                      return (
                        <div key={ev._id} className="ev-row" style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 9, border: "1px solid #f3f4f6", transition: "background .12s" }}>
              ...
              <div>
                ...
                <div style={{ background: "#fff", borderRadius: 14, border: `1.5px solid ${cfg.border}`, padding: "20px", marginBottom: 20, display: "flex", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    {!status && <button onClick={() => handleJoin(activeClub)} disabled={joiningClub === activeClub} style={{ padding: "9px 18px", background: cfg.color, border: "none", borderRadius: 9, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}><UserPlus size={14}/>{joiningClub === activeClub ? "Joining..." : "Join Club"}</button>}
                    {status === "approved" && <button onClick={() => handleLeave(activeClub)} style={{ padding: "9px 14px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 9, color: "#dc2626", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}><UserMinus size={13}/>Leave</button>}
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  ...
                  <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: "18px 20px" }}>
                    ...
                    {pts.length === 0 ? (
                      <p style={{ color: "#9ca3af", fontSize: 13, textAlign: "center", padding: "10px 0" }}>Join & attend events to earn points!</p>
                    ) : (
