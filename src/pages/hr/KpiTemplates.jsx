...
// ✅ Added "count" and "value" units
const UNITS       = ["tasks", "count", "value", "₹", "%", "hours", "tickets", "calls", "deals", "score"];
const FREQUENCIES = ["daily", "weekly", "monthly", "quarterly"];

// ✅ KPI Owner options — who fills actual values for this KPI
const KPI_OWNERS = [
...
// ✅ owner_role added to defaultItem
const defaultItem = { kpi_name: "", target: "", unit: "tasks", weight: "", frequency: "monthly", owner_role: "self" };
...
function OwnerBadge({ role }) {
  ...
  return (
    <span className="owner-badge" style={{ background: s.bg, color: s.color }}>
      {label}
    </span>
  );
}
...
export default function KpiTemplates() {
  ...
  const handleDeptChange = (deptName) => {
    ...
    if (found) {
      setRoles((found.designations || []).filter(dg => dg.status === "active"));
    } else {
      setRoles([]);
    }
  };
  ...
  const openEdit = (t) => {
    setForm({
      ...
      // ✅ Ensure owner_role is preserved when editing
      kpi_items: t.kpi_items.map(i => ({ ...defaultItem, ...i }))
    });
    setEditingId(t._id);
    ...
    <div className="kpi-page" style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: "100vh", background: "#f4f6fb" }}>
              ...
              <div style={{ padding: "16px 20px" }}>
                ...
                {t.kpi_items?.slice(0, 3).map((item, i) => (
                  <div key={i} className="kpi-card-kpi-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #f3f4f6", gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ fontSize: 13, color: "#374151", fontWeight: 500, display: "block", wordBreak: "break-word" }}>{item.kpi_name}</span>
                      {/* ✅ Show owner badge on card */}
                      {item.owner_role && item.owner_role !== "self" && (
                        <OwnerBadge role={item.owner_role} />
                      )}
                    </div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 720, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
                ...
                <div>
                  ...
                  {roles.length > 0 ? (
                    ...
                    <input
                      value={form.role}
                      onChange={e => handleFormChange("role", e.target.value)}
                      placeholder={!form.department ? "Select a department first" : "No roles found — type manually"}
                      style={{ ...inputStyle, background: !form.department ? "#f9fafb" : "#fff", color: !form.department ? "#9ca3af" : "#1a1a2e" }}
                      disabled={!form.department}
                    />
                  )}
                  {form.department && roles.length === 0 && (
                    <p style={{ margin: "4px 0 0", fontSize: 11, color: "#9ca3af" }}>No active roles — type manually</p>
                  )}
                </div>

                {/* ✅ Description — now mandatory (full width) */}
                <div style={{ gridColumn: "1 / -1" }}>
                  <label style={labelStyle}>
                    Description *
                    <span style={{ color: "#ef4444", marginLeft: 4, fontWeight: 400 }}>Required</span>
                  </label>
                  <input
                    value={form.description}
                    onChange={e => handleFormChange("description", e.target.value)}
                    placeholder="Describe the purpose of this KPI template (required)"
                    style={{
                      ...inputStyle,
                      borderColor: !form.description?.trim() ? "#fca5a5" : "#d1d5db"
                    }}
                  />
                  {!form.description?.trim() && (
                    <p style={{ margin: "4px 0 0", fontSize: 11, color: "#f59e0b" }}>⚠ Description is required</p>
                  )}
                </div>
              </div>
                  ...
                  <div key={idx} style={{ background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: 10, padding: 16, marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        {/* ✅ Show owner badge in header of each item */}
                    ...
                    {/* ✅ 6-column grid: name, target, unit, weight, frequency, owner */}
                    ...
                    {/* ✅ Hint for non-self owners */}
                    {item.owner_role && item.owner_role !== "self" && (
                      <div style={{ marginTop: 8, padding: "6px 10px", background: "#fefce8", border: "1px solid #fde68a", borderRadius: 6, fontSize: 12, color: "#92400e" }}>
                        ⚠ This KPI's actual value will be entered by <strong>{KPI_OWNERS.find(o => o.value === item.owner_role)?.label}</strong>, not the employee.
                      </div>
                    )}
                  </div>
                ))}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, minWidth: 480 }}>
                  <thead>
                    <tr style={{ background: "#f8fafc" }}>
                      {["KPI Name", "Target", "Unit", "Weight", "Frequency", "Filled By"].map(h => (
                ...
                {viewTemplate.kpi_items.map((item, i) => (
                  <div key={i} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "12px 14px", marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, gap: 8 }}>
                      ...
                      <span style={{ background: "#eff6ff", color: "#2563eb", fontWeight: 700, padding: "3px 8px", borderRadius: 4, fontSize: 12, flexShrink: 0 }}>{item.weight}%</span>
                    </div>
