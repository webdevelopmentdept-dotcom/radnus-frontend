...
const STYLES = `
  ...
  .ak-main-table tbody tr:nth-child(even) td {
    background: #fafafa;
  }
  .ak-main-table tbody tr:hover td {
    background: #f0f6ff;
  }
  .ak-action-btns {
    display: flex;
  ...
  @media (max-width: 768px) {
    .ak-page { padding: 16px; }
    .ak-header { flex-direction: column !important; align-items: flex-start !important; gap: 12px; }
    .ak-header-btn { width: 100%; text-align: center; justify-content: center; }
...
export default function AssignKpi() {
  ...
  const handleSubmit = async () => {
    if (!form.employee_id || !form.template_id) return showToast("Please select employee and template", "error");
    setSaving(true);
    try {
      const payload = {
      ...
      if (editingAssignment) {
        res = await axios.put(`${API_BASE}/api/kpi-assignments/${editingAssignment._id}`, payload);
      } else {
        res = await axios.post(`${API_BASE}/api/kpi-assignments`, payload);
      }
  ...
  return (
    <div className="ak-page" style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: "100vh", background: "#f4f6fb" }}>
      ...
      <div className="ak-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, gap: 12 }}>
        ...
        <button className="ak-header-btn" onClick={() => setShowModal(true)} style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
          + Assign KPI
        </button>
      </div>
      ...
      <div className="ak-stats" style={{ display: "grid", gap: 16, marginBottom: 24 }}>
        {[
          { label: "Total Assignments", value: assignments.length, color: "#2563eb", bg: "#eff6ff", icon: "📋" },
          { label: "Active", value: assignments.filter(a => a.status === "active").length, color: "#16a34a", bg: "#f0fdf4", icon: "✅" },
          { label: "Completed", value: assignments.filter(a => a.status === "completed").length, color: "#d97706", bg: "#fffbeb", icon: "🏁" },
        ].map((s, i) => (
                  ...
                  {assignments.map((a, i) => {
                    ...
                    return (
                      <tr key={a._id}>
                        ...
                        {/* ✅ Action — nowrap buttons */}
                        <td>
                          <div className="ak-action-btns">
                            <button
                              onClick={() => handleEdit(a)}
                              style={{ background: "#eff6ff", color: "#2563eb", border: "1px solid #bfdbfe", borderRadius: 6, padding: "4px 11px", fontSize: 11, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                            >Edit</button>

                            {a.status === "active" && (
                              <button
                                onClick={() => setCancelConfirm(a._id)}
                                style={{ background: "#fff7ed", color: "#d97706", border: "1px solid #fed7aa", borderRadius: 6, padding: "4px 11px", fontSize: 11, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                              >Cancel</button>
                            )}

                            <button
                              onClick={() => setDeleteConfirm(a._id)}
                              style={{ background: "#fff5f5", color: "#ef4444", border: "1px solid #fecaca", borderRadius: 6, padding: "4px 11px", fontSize: 11, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                            >Delete</button>
                          </div>
                        </td>
                      </tr>
                  ...
                  <div key={a._id} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: "14px 16px", background: "#fff" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#f3f4f6", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#6b7280", fontSize: 12, flexShrink: 0 }}>
                          {i + 1}
                        </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px", fontSize: 13, marginBottom: 10 }}>
                      ...
                      <div>
                        ...
                        <p style={{ margin: "2px 0 0", color: "#1a1a2e", fontWeight: 600 }}>{a.template_id?.template_name || "—"}</p>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => handleEdit(a)} style={{ flex: 1, padding: "8px 0", background: "#eff6ff", color: "#2563eb", border: "1px solid #bfdbfe", borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                        Edit
                      </button>
                      {a.status === "active" && (
                        <button onClick={() => setCancelConfirm(a._id)} style={{ flex: 1, padding: "8px 0", background: "#fff7ed", color: "#d97706", border: "1px solid #fed7aa", borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                          Cancel
                        </button>
                      )}
                      <button onClick={() => setDeleteConfirm(a._id)} style={{ flex: 1, padding: "8px 0", background: "#fff5f5", color: "#ef4444", border: "1px solid #fecaca", borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                        Delete
                      </button>
                    </div>
                  </div>
