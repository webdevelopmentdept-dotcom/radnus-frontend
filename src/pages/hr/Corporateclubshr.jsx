...
const STYLES = `
  ...
  .cc-tabs { display: flex; gap: 4px; background: #fff; border-radius: 10px; padding: 4px; border: 1px solid #e5e7eb; margin-bottom: 20px; width: fit-content; }
  .cc-tab { padding: 8px 18px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 13px; transition: all .15s; background: transparent; color: #6b7280; }
...
export default function CorporateClubs() {
  ...
  const [activeTab,  setActiveTab]        = useState("overview"); // overview | members | events | points
  ...
  // ── Event Actions ───────────────────────────────────────────────────────────
  const openCreateEvent = () => { setEventForm(blankEvent); setEditEvent(null); setShowEventForm(true); setActiveTab("events"); };
  const openEditEvent   = (ev) => { setEventForm({ title: ev.title, description: ev.description || "", date: ev.date?.slice(0,10) || "", venue: ev.venue || "", points_awarded: ev.points_awarded || 10, status: ev.status, max_participants: ev.max_participants || "" }); setEditEvent(ev._id); setShowEventForm(true); };
  ...
  const handleSaveEvent = async () => {
    ...
    try {
      ...
      setShowEventForm(false); setEditEvent(null); setEventForm(blankEvent); fetchAll();
  ...
  const handleAwardPoints = async () => {
    if (!pointsForm.employee_id || !pointsForm.points) return showMsg("Employee and points required", "error");
    setSaving(true);
    try {
      await axios.post(`${API_BASE}/api/clubs/${activeClub}/points`, pointsForm);
      showMsg("Points awarded! ⭐"); setShowPointsForm(false); setPointsForm(blankPoints); fetchAll();
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    ...
                    <tbody>
                      {members.map((m, i) => {
                        ...
                        const pts = pointsLog.filter(p => (p.employee_id?._id || p.employee_id) === (m.employee_id?._id || m.employee_id)).reduce((s,p) => s+(p.points||0), 0);
                        return (
              ...
              {showEventForm && (
                <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", marginBottom: 20, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: 20 }}>
                    ...
                    <div style={{ marginBottom: 16 }}><label style={lbl}>Description</label><textarea style={{ ...inp, minHeight: 60, resize: "vertical" }} value={eventForm.description} onChange={e => setEventForm(f => ({...f, description: e.target.value}))} placeholder="Event details..."/></div>
              ...
              {showPointsForm && (
                <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", marginBottom: 20, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: 20 }}>
                    <div className="cc-form-grid" style={{ display: "grid", gap: 14, marginBottom: 14 }}>
                      <div>
                        <label style={lbl}>Employee * (Active Only)</label>
