...
export default function ProductManagement() {
  // Lives under pages/hr/ — routed at /hr/dashboard/masters/products
  ...
  // ── Product Management module access (HR assigns this to one employee) ──
  const [allEmployees, setAllEmployees] = useState([]);
  const [assignedEmployee, setAssignedEmployee] = useState(null); // { _id, name, ... } or null
  ...
  // Logged-in HR user — used for "Updated By"
  const getHrUser = () => {
    try {
      const raw = localStorage.getItem("hrUser");
      if (raw) {
        const obj = JSON.parse(raw);
        return { id: obj?._id || obj?.id || obj?.hrId, name: obj?.name || "HR" };
      }
    } catch (_) {}
    return { id: localStorage.getItem("hrId") || null, name: "HR" };
  };
  ...
  // Every /api/products and /api/employee call now needs this — backend
  // added `auth` + `canManageProducts` middleware, so requests without a
  // valid HR token get a 401.
  const authHeaders = () => ({
  ...
  const loadCurrentAssignee = async () => {
    try {
      const res = await fetch(`${API}/api/employee/product-access/current`, { headers: authHeaders() });
      const data = await res.json();
      if (data.success) setAssignedEmployee(data.data || null);
    } catch (err) {
      console.error("LOAD ASSIGNEE ERROR", err);
    }
  };

  const assignProductAccess = async () => {
    if (!selectedAssigneeId) return;
    setAssigning(true);
    try {
      const res = await fetch(`${API}/api/employee/${selectedAssigneeId}/product-access`, {
        method: "PUT",
        headers: authHeaders(),
      });
      const data = await res.json();
      if (data.success) {
        ...
        alert(data.message || "Could not assign access");
      }
    } catch (err) {
      console.error("ASSIGN ERROR", err);
      alert("Could not assign access. Please try again.");
    } finally {
      setAssigning(false);
    }
  };
  ...
  const removeProductAccess = async () => {
    if (!assignedEmployee?._id) return;
    if (!window.confirm(`Remove Product Management access from ${assignedEmployee.name}?`)) return;
    try {
      const res = await fetch(`${API}/api/employee/${assignedEmployee._id}/product-access`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      const data = await res.json();
      if (data.success) setAssignedEmployee(null);
    } catch (err) {
  ...
  const saveProduct = async (e) => {
    ...
    // Store the readable name, not the raw Mongo id — updatedBy is a plain
    ...
    payload.append("updatedBy", hrUser.name || hrUser.id || "");
  ...
  return (
    <div className="pm-root container-fluid px-4 py-4">
      ...
      {/* PRODUCT ACCESS — HR assigns this module to exactly one employee */}
      <div className="pm-card" style={{ padding: "16px 20px", marginBottom: "18px" }}>
        <div className="pm-section-eyebrow" style={{ marginBottom: "10px" }}>Product Management Access</div>
        {assignedEmployee ? (
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div>
              Currently assigned to <strong>{assignedEmployee.name}</strong>
              {assignedEmployee.department ? ` — ${assignedEmployee.department}` : ""}
            </div>
            <button className="pm-btn pm-btn-outline-danger pm-btn-sm" onClick={removeProductAccess}>
              Remove Access
            </button>
          </div>
        ) : (
          <div className="d-flex align-items-center gap-2 flex-wrap">
            <span className="pm-hint" style={{ marginRight: "4px" }}>No employee assigned yet.</span>
            <select
              className="pm-select"
