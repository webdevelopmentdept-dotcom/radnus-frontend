...
// Added: Salary Scale Point selector (Min / Mid / Max) in assign/edit modal
...
// Scale point config
const SCALE_POINTS = [
  { key: "min", label: "Minimum", short: "Min", color: "#059669", bg: "#ecfdf5", border: "#6ee7b7", desc: "Entry of band" },
  { key: "mid", label: "Midpoint", short: "Mid", color: "#2563eb", bg: "#eff6ff", border: "#93c5fd", desc: "Market reference / target" },
  { key: "max", label: "Maximum", short: "Max", color: "#d97706", bg: "#fff7ed", border: "#fcd34d", desc: "Ceiling of band" },
];

const fmt = (v) => v ? `₹${Number(v).toLocaleString("en-IN")}` : null;

const getScaleValue = (grade, point) => {
  if (!grade) return null;
  if (point === "min") return grade.salary_band_min;
  if (point === "mid") return grade.salary_band_mid;
  if (point === "max") return grade.salary_band_max;
  return null;
};
...
const defaultForm = { employee_id: "", grade_id: "", effective_date: "", reason: "", salary_scale_point: "" };
...
function SalaryScaleSelector({ grade, value, onChange }) {
  const hasScale = grade && (grade.salary_band_min || grade.salary_band_mid || grade.salary_band_max);
  if (!grade) return null;
  if (!hasScale) {
    return (
  ...
  // Which dot position on the track
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div className="scale-radio-group">
        {SCALE_POINTS.map(sp => {
          const val = getScaleValue(grade, sp.key);
          if (!val) return null;
      ...
      <div style={{ marginTop: 10, padding: "10px 14px", background: "#f8fafc", borderRadius: 8, border: "1px solid #e5e7eb" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#9ca3af", marginBottom: 2 }}>
          <span>{fmt(grade.salary_band_min) || "—"}</span>
          <span>{fmt(grade.salary_band_mid) || "—"}</span>
          <span>{fmt(grade.salary_band_max) || "—"}</span>
        </div>
        <div className="track-bar">
          ...
          {dotLeft && (
            <div
              ...
              style={{
                left: dotLeft,
                color: SCALE_POINTS.find(s => s.key === value)?.color || "#2563eb",
                background: SCALE_POINTS.find(s => s.key === value)?.color || "#2563eb",
              }}
            />
          )}
        </div>
        ...
        {value && (
          <div style={{ marginTop: 8, textAlign: "center", fontSize: 12 }}>
            ...
            <strong style={{ color: SCALE_POINTS.find(s => s.key === value)?.color }}>
              {SCALE_POINTS.find(s => s.key === value)?.label} — {fmt(getScaleValue(grade, value))}
            </strong>
          </div>
        )}
      </div>
  ...
  return (
    <div className="ag-page" style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: "100vh", background: "#f4f6fb" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                  <thead>
                    <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e5e7eb" }}>
                      {["Employee", "Emp ID", "Grade", "Designation", "BGR Stage", "Scale Point", "Effective Date", "Changes", "Actions"].map(h => (
                    ...
                    {filtered.map((a, i) => {
                      ...
                      return (
                        <tr key={a._id}
                          ...
                          {/* Scale Point column — NEW */}
                          <td style={{ padding: "14px 16px" }}>
                            <ScalePointBadge point={a.salary_scale_point} />
                          </td>
      ...
      {viewItem && (() => {
        ...
        const spVal = getScaleValue(viewItem.grade_id, viewItem.salary_scale_point);
        return (
              ...
              <div style={{ padding: 24 }}>
                {viewTab === "details" && (
                  <>
                    ...
                    {/* Salary Scale Point — NEW in view */}
                    {(viewItem.grade_id?.salary_band_min || viewItem.grade_id?.salary_band_mid || viewItem.grade_id?.salary_band_max) && (
                      <div style={{ marginBottom: 16, padding: "14px 16px", background: "#f8fafc", borderRadius: 10, border: "1px solid #e5e7eb" }}>
                        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 10 }}>
                          Salary Scale
                        </label>
                        {/* 3 point pills */}
                        <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                          {SCALE_POINTS.map(sp => {
                            const val = getScaleValue(viewItem.grade_id, sp.key);
                        ...
                        {spMeta && spVal && (
                          <div style={{ marginTop: 8, textAlign: "center", fontSize: 13, fontWeight: 700, color: spMeta.color }}>
                            Employee sits at {spMeta.label}: {fmt(spVal)}
                          </div>
                        )}
                        {!viewItem.salary_scale_point && (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 580, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            <div style={{ padding: 24 }}>
              ...
              {/* ── Salary Scale Point Selector — NEW ── */}
              <SalaryScaleSelector
                grade={selectedGrade}
